...
app.get('/api', (req, res) => {
  ...
  res.json({
    ...
    endpoints: [
      ...
      {method: 'PUT', path: '/api/books/:id', description: 'Delete a book based upon the specified ID'},
      {method: 'DELETE', path: '/api/books/:id', description: 'Update a book information based upon the specified ID'},
    ]
  })
});
app.get('/api/profile', (req, res) => {
  res.json({
    'name': 'Jakob Zurstiege',
    'homeCountry': 'Germany',
    'degreeProgram': 'MMT',//informatics or CSE.. etc
    'email': 'jakob@zurstiege.de',
...
app.put('/api/books/:id', async (req, res) => {
  ...
  const updatedBookInfo = await db.books.findOneAndUpdate(
    {_id: bookId},
    bookNewData,
    {new: true}
  );
...
app.delete('/api/books/:id', async (req, res) => {
   ...
   */
  const bookId = req.params.id;
  const deletedBook = await db.books.findOneAndRemove(
    {_id: bookId}
  );
